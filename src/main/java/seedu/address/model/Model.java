...
public interface Model {
    ...
    Comparator<Attraction> COMPARATOR_SORT_BY_PRIORITY_DESCENDING = (attr1, attr2)
            -> Integer.compare(Integer.parseInt(attr2.getPriority().value),
            Integer.parseInt(attr1.getPriority().value));
    ...
    Comparator<Itinerary> COMPARATOR_SORT_BY_CUMULATIVE_ITINERARY_PRIORITY_DESCENDING = (itinerary1, itinerary2)
            -> Integer.compare(itinerary2.getCumulativeItineraryPriority(),
            itinerary1.getCumulativeItineraryPriority());
    ...
    /**
     * Returns true if an attraction with the same identity as
     * {@code attraction} exists in the Maplet.
     */
    boolean hasAttraction(Attraction attraction);

    /**
     * Returns true if an attraction with the same identity as {@code location}
     * exists in the Maplet.
     */
    boolean hasLocation(Location location);

    /**
     * Returns true if a location with the same name as {@code locationName}
     * exists in the Maplet.
     */
    ...
    /**
     * Deletes the given attraction. The attraction must exist in the Maplet.
     */
    void deleteAttraction(Attraction target);

    /**
     * Deletes the location with {@code locationName}. The location must exist
     * in the Maplet.
     */
    void deleteLocation(LocationName locationName);

    /**
     * Adds the given attraction. {@code attraction} must not already exist in
     * the Maplet.
     */
    void addAttraction(Attraction attraction);

    /**
     * Adds the given location. {@code location} must not already exist in the
     * Maplet.
     */
    ...
    /**
     * Replaces the given attraction {@code target} with
    ...
    /**
     * Updates the filter of the filtered attraction list to filter by the given
     * {@code predicate}.
     *
     ...
     * Sets the active tab
     */
    void setActiveTab(ActiveTab activeTab);
